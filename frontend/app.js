...
function updateSessionExecutionUI() {
  ...
  const canPreview = ui.isPaper || (ui.isLive && ui.sessionConnected && ui.unlocked);
  ...
  if (preview) {
    ...
    preview.textContent = ui.isPaper ? 'Preview Paper Trade' : 'Preview Order';
    preview.title = ui.isPaper
      ? 'Preview the simulated paper trade for this session.'
      : ui.sessionConnected
  ...
  if (place) {
    ...
    place.textContent = ui.isPaper ? 'Execute Paper Trade' : 'Place Live Order';
    place.title = canPlace
      ? (ui.isPaper ? 'Record the last previewed paper trade in this browser session.' : 'Place the last previewed live order after confirmation.')
...
function renderSessionExecLog() {
  ...
  if (!_sessionExecEvents.length) {
    ...
    const msg = ui.isPaper
      ? 'Paper mode is active. Preview and execute simulated trades for this session.'
      : !ui.sessionConnected
...
async function previewSessionOrder() {
  const ui = getUiState();
  try {
    appendSessionExecEvent('preview', ui.isPaper ? 'Requesting paper preview' : 'Requesting dry-run proposal');
    ...
    _lastSessionProposal = data.proposal || null;
    appendSessionExecEvent('preview', ui.isPaper ? 'Paper preview ready' : 'Dry-run ready', proposalText(_lastSessionProposal));
    updateSessionExecutionUI();
    showMToast(ui.isPaper ? 'Paper preview ready' : 'Order preview ready');
  } catch(e) {
...
async function placeSessionOrder() {
  ...
  if (!_lastSessionProposal) {
    showMToast(ui.isPaper ? 'Preview a paper trade first' : 'Preview an order first');
    return;
  }
  let phrase = '';
  if (ui.isLive) {
    phrase = prompt('This can place a real Bitget futures order. Type PLACE LIVE ORDER to continue.');
    if (!phrase) return;
  }
  try {
    appendSessionExecEvent(ui.isPaper ? 'paper' : 'live', ui.isPaper ? 'Executing paper trade' : 'Submitting live order', proposalText(_lastSessionProposal));
    const data = await fetchJson('/api/session/execute', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
    ...
    if (!data.ok || !data.executed) {
      ...
      const msg = result.reason || data.error || (ui.isPaper ? 'Paper trade was not executed' : 'Live order was not placed');
      appendSessionExecEvent('blocked', msg, proposalText(data.proposal || _lastSessionProposal));
      showMToast(msg);
      return;
    }
    appendSessionExecEvent(ui.isPaper ? 'paper' : 'live', ui.isPaper ? 'Paper trade executed' : 'Live order placed', proposalText(data.proposal || _lastSessionProposal));
    showMToast(ui.isPaper ? 'Paper trade executed' : 'Live order placed');
    if (data.account) {
      rememberPaperEquity(data.account.equity);
      renderHeaderBalance({session: data.session, account: data.account});
    }
    loadTradeHistory();
  } catch(e) {
    appendSessionExecEvent('blocked', e.name === 'AbortError' ? 'Execution timed out' : 'Execution failed');
    showMToast(e.name === 'AbortError' ? 'Execution timed out' : (e.message || 'Execution failed'));
  }
}
...
function returnHome() {
  try { localStorage.removeItem(ENTERED_PLATFORM_KEY); } catch(e) {}
