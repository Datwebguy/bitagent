...
def _append_session_trade(session: dict, trade: dict):
    trades = session.setdefault("paper_trades", [])
    trades.append(trade)
    session["paper_trades"] = trades[-200:]


def _session_trade_proposal(symbol: str, decision: dict, balance: float) -> dict:
    direction = str(decision.get("direction") or "FLAT").upper()
    price = float(decision.get("entry_price") or 0)
    min_lot = _session_min_lot(symbol)
    size = 0.0
    if price > 0 and balance > 0:
        size = max(min_lot, round((balance * (EXEC_MAX_PCT / 100.0)) / price, 4))
    return {
        "symbol": symbol,
        "direction": direction,
        "confidence": int(decision.get("confidence") or 0),
...
def _session_execute_paper_trade(session: dict, symbol: str, decision: dict) -> dict:
    account = _session_paper_account(session, float(decision.get("entry_price") or 0), symbol)
    proposal = _session_trade_proposal(symbol, decision, account["equity"])
    direction = proposal["direction"]
    price = float(proposal["entry_price"] or 0)
    size = float(proposal["size"] or 0)
    confidence = int(proposal["confidence"] or 0)
    if direction not in ("LONG", "SHORT"):
        return {"executed": False, "action": "SKIP", "reason": "Paper execution requires a LONG or SHORT decision.", "proposal": proposal}
    if confidence < MIN_CONFIDENCE:
        return {"executed": False, "action": "SKIP", "reason": f"Confidence {confidence}% is below the {MIN_CONFIDENCE}% minimum.", "proposal": proposal}
    if price <= 0 or size <= 0:
        return {"executed": False, "action": "SKIP", "reason": "Paper execution needs a valid price and size.", "proposal": proposal}

    now = datetime.now(timezone.utc).isoformat()
    want_side = "long" if direction == "LONG" else "short"
    pos = session.get("paper_open_position") or {}
    curr_side = str(pos.get("side") or "").lower()
    curr_symbol = normalize_symbol(pos.get("symbol") or symbol) if pos else symbol
    realized_delta = 0.0

    if curr_side == want_side and curr_symbol == symbol:
        return {"executed": False, "action": "SKIP", "reason": f"Paper {want_side} position is already open for {symbol}.", "proposal": proposal}

    if curr_side in ("long", "short") and float(pos.get("size") or 0) > 0:
        old_size = float(pos.get("size") or 0)
        old_entry = float(pos.get("entry") or 0)
        close_price = price if curr_symbol == symbol else float(pos.get("mark") or old_entry or price)
        realized_delta = (close_price - old_entry) * old_size if curr_side == "long" else (old_entry - close_price) * old_size
        session["paper_realized"] = float(session.get("paper_realized") or 0) + realized_delta
        _append_session_trade(session, {
            "ts": now,
            "action": "CLOSE_PAPER",
            "symbol": curr_symbol,
            "side": curr_side,
            ...
            "pnl": round(realized_delta, 4),
            "audit": "Session paper close",
        })

    session["paper_open_position"] = {
        "symbol": symbol,
        "side": want_side,
        "size": size,
        "entry": price,
    ...
    _append_session_trade(session, {
        ...
        "audit": "Session paper fill",
    })
    _persist_session(session)
    return {
        "executed": True,
        "action": f"OPEN_{direction}_PAPER",
        "detail": f"paper {size} {symbol.replace('USDT', '')} @ ~${price:,.6g}",
        "size": size,
        "price": price,
        "pnl": round(realized_delta, 4),
        "proposal": proposal,
...
def _record_session_live_trade(session: dict, symbol: str, result: dict, proposal: dict):
    ...
    _append_session_trade(session, {
        ...
        "size": result.get("size") or proposal.get("size") or 0,
        "price": proposal.get("entry_price") or 0,
        "entry_price": proposal.get("entry_price") or 0,
        "order_id": result.get("order_id"),
        "confidence": proposal.get("confidence") or 0,
        "audit": "Session live fill",
    })
    _persist_session(session)
...
async def execute_session_trade(
    ...
    if session.get("trade_mode", "paper") != "live":
        ...
        result = _session_execute_paper_trade(session, symbol, decision)
        account = _session_paper_account(session, float(decision.get("entry_price") or 0), symbol)
        return {
            ...
            "account": account,
            "trades": _session_paper_trades(session, 50),
            "session": _session_payload(session),
        }
    ...
    loop = asyncio.get_running_loop()
    balance = await loop.run_in_executor(_executor, _session_futures_balance, creds)
    proposal = _session_trade_proposal(symbol, decision, balance)
    proposal["dry_run"] = req.dry_run
    if req.dry_run:
    ...
    _record_session_live_trade(session, symbol, result, proposal)
    return {
        "ok": bool(result.get("executed")),
        "executed": bool(result.get("executed")),
        "result": result,
